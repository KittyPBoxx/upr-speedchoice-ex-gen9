...
public class RomFunctions {
     ...
     */
    public static int[] getMovesAtLevel(Pokemon pkmn, int level) {
        return getMovesAtLevel(pkmn, level, 0);
    }

    public static int[] getMovesAtLevel(Pokemon pkmn, int level, int emptyValue) {
        ...
        int moveCount = 0;
        List<MoveLearnt> movepool = pkmn.getLearnset();
        for (MoveLearnt ml : movepool) {
            if (ml.getLevel() > level) {
                // we're done
                break;
            }
            ...
            if (!alreadyKnownMove) {
                ...
                if (moveCount == 4) {
                    ...
                    curMoves[3] = ml.getMove();
                } else {
                    // add to next available slot
                    curMoves[moveCount++] = ml.getMove();
                }
            }
        }
